...
// For details check our docs https://docs.0xkey.com/concepts/policies/delegated-access
package main
...
func main() {
	...
	apiKey, err := apikey.FromZeroXKeyPrivateKey("<private_key_here>", apikey.SchemeP256)
	if err != nil {
	...
	// Initializing a new 0xkey client used by the Delegated account activities
	// Notice the subOrganizationId created above
	delegatedApiKey, err := apikey.FromZeroXKeyPrivateKey(delegatedPrivateKey, apikey.SchemeP256)
	if err != nil {
		log.Fatal("creating API key: %w", err)
	}
