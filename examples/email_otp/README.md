...
[INIT_OTP](https://docs.0xkey.com/api-reference/activities/init-generic-otp),
[VERIFY_OTP](https://docs.0xkey.com/api-reference/activities/verify-generic-otp), and
[OTP_LOGIN](https://docs.0xkey.com/api-reference/activities/login-with-otp) activities.

## Security model

The OTP code and client public key are HPKE-encrypted together to 0xkey's Signer
...
8. Session JWT signature is verified against the configured notarizer public key.
...
The encryption target bundle returned by `INIT_OTP` is a signed JSON envelope from the Signer enclave:

```json
{
  "version": "v1.0.0",
  "data": "<hex(JSON: { targetPublic, organizationId, userId })>",
  "dataSignature": "<ECDSA signature over data>",
  "enclaveQuorumPublic": "<Signer quorum public key>"
}
```
...
The `dataSignature` (verified against the configured `-signer-public-key`)
proves this ephemeral key genuinely came from the real enclave — not a compromised proxy
substituting its own key.

## Setup

Follow the [Quickstart](https://docs.0xkey.com/getting-started/quickstart) to obtain:
...
For local `0xkey.com` testing, pass the local API endpoint and local enclave keys:

```bash
go run main.go \
  -api-host localhost:8080 \
  -api-scheme http \
  -signer-public-key "$(cat /tmp/0xkey-enclave-dev/pubkeys/signer.pubkey)" \
  -notarizer-public-key "$(cat /tmp/0xkey-enclave-dev/pubkeys/notarizer.pubkey)" \
  -api-private-key "your_api_private_key" \
  -parent-org-id "parent_org_id" \
  -sub-org-id "sub_org_id" \
  -email "user@example.com"
...
5. Print the session JWT and verify its signature against the configured notarizer key
