...
//
// Security model:
// The OTP code and client public key are HPKE-encrypted together to 0xkey's TLS Fetcher
...
import (
	...
	apiClient "github.com/0xkey-io/sdk-go/pkg/api/client"
	...
	"github.com/0xkey-io/sdk-go/pkg/util"
)
...
func init() {
	flag.StringVar(&apiPrivateKey, "api-private-key", "", "0xkey API private key for authentication")
	...
	flag.StringVar(&signerPubKey, "signer-public-key", encryptionkey.SignerProductionPublicKey, "Signer enclave quorum public key")
	flag.StringVar(&notarizerKey, "notarizer-public-key", crypto.ProductionNotarizerPublicKey, "Notarizer enclave quorum public key")
	flag.StringVar(&apiHost, "api-host", apiClient.DefaultHost, "0xkey API host")
	flag.StringVar(&apiScheme, "api-scheme", apiClient.DefaultSchemes[0], "0xkey API scheme")
}
...
func run() error {
	...
	clientPubHex := clientAPIKey.EncodedPublicKey
...
func initClient() (*sdk.Client, error) {
	apiKey, err := apikey.FromZeroXKeyPrivateKey(apiPrivateKey, apikey.SchemeP256)
	if err != nil {
		return nil, fmt.Errorf("failed to create API key: %w", err)
	}

	transport := apiClient.DefaultTransportConfig()
	transport.Host = apiHost
	transport.Schemes = []string{apiScheme}

	client, err := sdk.New(sdk.WithAPIKey(apiKey), sdk.WithTransportConfig(*transport))
	if err != nil {
...
// an ephemeral HPKE public key (targetPublic) generated fresh by the Signer enclave
// for this session, signed with the enclave's quorum key. The client verifies that
// signature (against the pinned SignerProductionPublicKey) before encrypting,
...
func verifyOTP(client *sdk.Client, otpID, otpCode, encryptionTargetBundle, clientPubHex string) (string, error) {
	signerKeyBytes, err := hex.DecodeString(signerPubKey)
	if err != nil {
		return "", fmt.Errorf("failed to decode Signer quorum key: %w", err)
	}
	signerKey, err := enclave_encrypt.ToEcdsaPublic(signerKeyBytes)
	if err != nil {
		return "", fmt.Errorf("failed to parse Signer quorum key: %w", err)
	}
...
// ParseVerificationTokenID extracts the token ID from a 0xkey verification token (JWT).
...
func loginOTP(client *sdk.Client, verificationToken string, clientPrivKey *ecdsa.PrivateKey, clientPubHex string) error {
	...
	if err := crypto.VerifySessionJwtSignature(*sessionJWT, notarizerKey); err != nil {
