# 0xkey + go-ethereum examples

This directory shows how to integrate 0xkey as the signer for Ethereum transactions using [`go-ethereum`](https://github.com/ethereum/go-ethereum), across three common integration levels:
...
These examples mirror real-world integration paths people take when adopting 0xkey:
...
- format it the way 0xkey expects,
- sign it with 0xkey
...
Use abigen-generated bindings to encode contract calls and submit them with a 0xkey-backed signer.

This example shows how to:
- initialize the generated ERC-20 wrapper,
- pack calldata with methods such as `erc.PackTransfer(to, amount)`,
- plug a 0xkey-powered `SignerFn` into `bind.TransactOpts`,
- build a normal EIP-1559 transaction containing that calldata,
- let 0xkey sign it inside the `SignerFn`,
...
**Goal:**
Manually build an unsigned EIP-1559 transaction, RLP-encode it in the exact format 0xkey expects, sign it with 0xkey, and broadcast it.
...
- Signs the resulting hex string with 0xkey.
...
**Goal:**
Provide a reusable 0xkey-backed `bind.SignerFn` that can sign any EIP-1559 transaction created by `go-ethereum`.

**How it works:**

- Defines `MakeZeroXKeySignerFn(client, signWith, chainID)` which returns a `bind.SignerFn` compatible with `accounts/abi/bind/v2`.
- Inside the signer:
  - verifies the transaction’s `from` address matches the 0xkey signer address,
  - ensures the transaction is EIP-1559 (`DynamicFeeTxType`),
  - injects the correct chain ID if omitted by upstream code,
  - builds the unsigned EIP-1559 payload 0xkey expects,
  - RLP-encodes it and prepends the `0x02` type byte,
  - sends the payload to 0xkey’s `SignTransactionV2`,
...
**Goal:**
Use abigen-generated contract bindings (v2) together with a 0xkey-backed signer, allowing you to encode typed contract calls while 0xkey signs the resulting transaction.
...
```
- Constructs `bind.TransactOpts` using the same `MakeZeroXKeySignerFn`.

```go
auth := &bind.TransactOpts{
    From:   zeroXKeyAddress,
    Signer: signerFn,
}
```
...
- The transaction is then passed into your SignerFn, which sends the unsigned payload to 0xkey and returns a fully signed transaction.
...
- 0xkey signs it via your `SignerFn`,
...
- A 0xkey [organization](https://docs.0xkey.com/getting-started/quickstart) with:
  - an API key (P-256),
...
```bash
cd sdk-go/examples/go-ethereum
cp .env.example .env
```
...
```bash
ZEROXKEY_ORGANIZATION_ID="<0xkey organization ID>"
ZEROXKEY_API_PUBLIC_KEY="<0xkey API public key (starts with 02 or 03)>"
ZEROXKEY_API_PRIVATE_KEY="<0xkey API private key>"

RPC_URL="<Ethereum RPC provider URL>"
SIGN_WITH="<0xkey-managed Ethereum address>"
