...
import (
	...
	"github.com/0xkey-io/sdk-go/examples/go-ethereum/bindsigner-abigen/erc20"
...
func main() {
	...
	orgID := os.Getenv("ZEROXKEY_ORGANIZATION_ID")
	privateKey := os.Getenv("ZEROXKEY_API_PRIVATE_KEY")
	contractAddress := os.Getenv("CONTRACT_ADDRESS")

	if rpcURL == "" || signWith == "" || orgID == "" || privateKey == "" || contractAddress == "" {
		log.Fatal("RPC_URL, SIGN_WITH, ZEROXKEY_ORGANIZATION_ID, ZEROXKEY_API_PRIVATE_KEY, and CONTRACT_ADDRESS must be set")
	}

	fromAddress := common.HexToAddress(signWith)

	// -------------------------------------------------------------------------
	// 2) 0xkey API key + SDK client
	// -------------------------------------------------------------------------
	apiKey, err := apikey.FromZeroXKeyPrivateKey(privateKey, apikey.SchemeP256)
	if err != nil {
	...
	// -------------------------------------------------------------------------
	// 4) 0xkey-backed bind/v2 TransactOpts (SignerFn)
	// -------------------------------------------------------------------------
	signerFn := MakeZeroXKeySignerFn(client, signWith, chainID, orgID)
	...
	fmt.Println("→ Calling ERC20.transfer via abigen v2 + 0xkey...")

	// -------------------------------------------------------------------------
	// 7) Let the 0xkey-backed SignerFn sign the tx
	// -------------------------------------------------------------------------
	signedTx, err := auth.Signer(fromAddress, unsignedTx)
	if err != nil {
		log.Fatalf("0xkey signer error: %v", err)
	}
...
// MakeZeroXKeySignerFn returns a bind/v2.SignerFn that:
//
//   - normalizes the transaction (ensures chainID is set),
//   - builds the EIP-1559 unsigned payload 0xkey expects,
//   - calls SignTransactionV2,
//   - returns a fully signed *types.Transaction.
func MakeZeroXKeySignerFn(
	client *sdk.Client,
	signWith string,
	chainID *big.Int,
	orgID string,
	...
	return func(from common.Address, tx *types.Transaction) (*types.Transaction, error) {
		// Ensure signer address matches the 0xkey address
		...
		unsignedHex := hex.EncodeToString(unsigned) // 0xkey expects hex without 0x

		// Prepare 0xkey SignTransactionV2 request
		...
		if err != nil {
			return nil, fmt.Errorf("0xkey SignTransactionV2 error: %w", err)
		}
		...
		signedHex := resp.Payload.Activity.
			...
			SignedTransaction
		if signedHex == nil {
			return nil, fmt.Errorf("0xkey returned nil signed transaction")
		}
