...
func main() {
	...
	if rpcURL == "" || signWith == "" || orgID == "" || privateKey == "" {
		log.Fatal("missing RPC_URL, SIGN_WITH, ZEROXKEY_ORGANIZATION_ID, ZEROXKEY_API_PRIVATE_KEY")
	}

	fromAddress := common.HexToAddress(signWith)

	// ---------------------------------------------------------------------
	// 0xkey API Key and SDK client
	// ---------------------------------------------------------------------
...
func MakeZeroXKeySignerFn(
	...
	return func(from common.Address, tx *types.Transaction) (*types.Transaction, error) {
		...
		if err != nil {
			return nil, fmt.Errorf("0xkey signing error: %w", err)
		}

		signedHex := resp.Payload.Activity.Result.SignTransactionResult.SignedTransaction
		if signedHex == nil {
			return nil, fmt.Errorf("nil signed transaction from 0xkey")
		}
