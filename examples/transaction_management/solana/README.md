...
Demonstrates 0xkey's [transaction management](https://docs.0xkey.com/concepts/transaction-management) feature for Solana with both sponsored and non-sponsored modes.
...
All actions except `assets` use 0xkey Gas Station for gas sponsorship by default. Pass `-sponsor=false` to use non-sponsored mode.

## What sponsorship handles

Beyond simple fee-payer substitution, 0xkey's Solana sponsorship handles:
...
The user can submit a transaction with no signatures — 0xkey signs with both the user's key and the paymaster.

## Prerequisites

- A 0xkey organization entitled to use the Transaction Management feature
...
4. **Sponsored mode** (`-sponsor=true`, default): uses a placeholder blockhash — 0xkey reconstructs the transaction with a fresh blockhash, substitutes the fee payer with a paymaster, and handles rent for any new accounts (e.g. ATA creation)
