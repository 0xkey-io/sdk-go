// Package main demonstrates Solana transaction management with 0xkey.
//
...
//
// All actions use 0xkey Gas Station for gas sponsorship by default.
...
func init() {
	flag.StringVar(&apiPrivateKey, "api-private-key", "", "0xkey API private key")
	flag.StringVar(&organizationID, "organization-id", "", "0xkey organization ID")
...
func initClient() (*sdk.Client, error) {
	key, err := apikey.FromZeroXKeyPrivateKey(apiPrivateKey, apikey.SchemeP256)
	if err != nil {
...
func sendSOL(client *sdk.Client) error {
	...
	// For sponsored mode, use a placeholder blockhash — 0xkey fetches a fresh one.
...
func sendToken(client *sdk.Client) error {
	...
	// In sponsored mode, 0xkey covers the rent-exemption fee.
...
// Jupiter builds the transaction; we just convert it to hex and submit to 0xkey.
func swapSOL(client *sdk.Client) error {
	...
	// 3. Convert the base64 transaction to hex and submit via 0xkey.
