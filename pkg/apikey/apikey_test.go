...
func Test_FromEncodedPrivateKey(t *testing.T) {
	...
	apiKey, err := apikey.FromZeroXKeyPrivateKey(privateKeyFromOpenSSL, apikey.SchemeP256)
	...
	assert.Equal(t, expectedPublicKey, apiKey.EncodedPublicKey)
}
...
func Test_Sign_ECDSA(t *testing.T) {
	...
	testCases := []testCase{
		{
			name:              "p256",
			encodedPrivateKey: "487f361ddfd73440e707f4daa6775b376859e8a3c9f29b3bb694a12927c0213c",
			encodedPublicKey:  "02f739f8c77b32f4d5f13265861febd76e7a9c61a1140d296b8c16302508870316",
			curve:             apikey.CurveP256,
		},
		{
			name:              "secp256k1",
			encodedPrivateKey: "1e0d32856eb059ee6c9d7871ac1c0755b7ecca4b6302263448da30d10c91c50d",
			encodedPublicKey:  "032f1d146fc0de39f093bcb9f0b9ce667030c2b8a3ad0c3022cfae2c6a7a21d28a",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(tt *testing.T) {
			apiKey, err := apikey.FromZeroXKeyPrivateKey(tc.encodedPrivateKey, tc.curve.ToScheme())
			...
			assert.Equal(tt, tc.encodedPublicKey, stamp.PublicKey)
			assert.Equal(tt, tc.curve.ToScheme(), stamp.Scheme)

			sigBytes, err := hex.DecodeString(stamp.Signature)
			require.NoError(tt, err)

			publicKey, err := apikey.DecodeZeroXKeyPublicECDSAKey(stamp.PublicKey, tc.curve.ToScheme())
			require.NoError(tt, err)
...
func Test_Sign_ED25519(t *testing.T) {
	...
	apiKey, err := apikey.FromZeroXKeyPrivateKey(encodedPrivateKey, apikey.SchemeED25519)
	require.NoError(t, err)
...
func Test_EncodedKeySizeIsFixed(t *testing.T) {
	for i := 0; i < 1000; i++ {
		...
		assert.Len(t, apiKey.EncodedPublicKey, 66, "attempt %d: expected 66 characters for public key %s", i, apiKey.EncodedPublicKey)
		assert.Len(t, apiKey.EncodedPrivateKey, 64, "attempt %d: expected 64 characters for private key %s", i, apiKey.EncodedPrivateKey)
	}
}
