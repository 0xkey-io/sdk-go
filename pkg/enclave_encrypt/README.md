...
- Client - a client outside of the enclave; normally a 0xkey end user.
...
This protocol builds on top of the HPKE standard ([RFC 9180](https://datatracker.ietf.org/doc/html/rfc9180)) by adding recipient pre-flight authentication so the client can verify it is sending ciphertext to a 0xkey controlled enclave and the enclave can verify its sending ciphertext to the correct client. See the [security profile](#security-profile) section more details.
...
1. Client generates target pair and sends clientTargetPub key to server. The authenticity of the clientTargetPub is assumed to have been verified by the policy engine.
...
1. Server assumes the authenticity of clientEncappedPub has been verified by the policy engine.
...
- Receiver pre-flight authentication: we achieve recipient authentication for both the server and client:
  ...
  - Server to Client: server relies on upstream checks by the policy engine and activity signing scheme to enforce rules that guarantee authenticity of the clients target key. Specifically, when the client "sends" clientTargetPub it actually submits a signed payload (activity), and that payload must be signed with an existing credential persisted in org data.
