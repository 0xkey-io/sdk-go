...
const (
	...
	testOrgID           = "f412ea93-998b-45a5-9df8-d2797c7f1a67"
	testUserID          = "4eb08a82-00ee-4f29-b076-fca769209725"
)
...
// This is a demonstration of the internals of our flow. Below we do not go
// through the abstractions we've built, which makes this test a good guide for
// anyone who wants to implement email auth bundle decryption in other languages.
func TestManualDecryptEmailBundle(t *testing.T) {
	...
	authKey, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	assert.Nil(t, err)
	client, err := enclave_encrypt.NewEnclaveEncryptClientFromTargetKey(&authKey.PublicKey, receiverPrivate)
	assert.Nil(t, err)
	clientTarget, err := client.TargetPublic()
	assert.Nil(t, err)
	server, err := enclave_encrypt.NewEnclaveEncryptServer(authKey, testOrgID, nil)
	assert.Nil(t, err)
	payload, err := server.AuthEncrypt(clientTarget, []byte("test message"))
	assert.Nil(t, err)

	suite := hpke.NewSuite(KemId, enclave_encrypt.KdfId, enclave_encrypt.AeadId)
	receiver, err := suite.NewReceiver(receiverPrivate, []byte(enclave_encrypt.ZeroXKeyHpkeInfo))
	assert.Nil(t, err)
...
func TestWalletExportWithV1BundlesEndToEnd(t *testing.T) {
	...
	expectedPublicKey = append(expectedPublicKey, enclaveKey.X.Bytes()...)
	expectedPublicKey = append(expectedPublicKey, enclaveKey.Y.Bytes()...)
